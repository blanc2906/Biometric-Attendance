...
import {
  ...
  ATTENDANCE_NOTI,
  FINGER_ATTENDANCE,
} from 'src/shared/constants/mqtt.constant';
...
export class UsersController {
  ...
  async addFace(
    ...
    try {
      ...
      const addedUser = await this.faceRecognitionService.addFaceDescriptor(
        id,
        imagePath,
      );
  ...
  async recognizeFaceFromCamera(@UploadedFile() file: Express.Multer.File) {
    ...
    try {
      ...
      const recognizedUser =
        await this.faceRecognitionService.recognizeFace(tempPath);
  ...
  private isWithinTimeThreshold(
    ...
    const diffInMinutes =
      Math.abs(currentTime.getTime() - prevDateTime.getTime()) / (1000 * 60);
